...
|---|---|---|
| Main steps 1-4 | A real staff HTTP journey searches and displays George Franklin, then submits owner and pet forms using only each target form's rendered CSRF field and verifies exact rendered and database values at `ClinicRecordsE2ETests.java:57` | PASS |
...
| RULE-9 | Exact Thymeleaf actions render a CSRF field in every owner, pet, and walk-in POST form; the real-server client extracts only that target-form token, while whole-route authorization, role matrix, owner 403, and no-mutation checks pass | PASS |
...
| RULE-18 | Four UC-8 real-server journeys and one concurrent real-server race cover every contract branch and prohibited side effect; target-form-only CSRF extraction prevents a token from another form from masking browser-incompatible markup | PASS |
...
- Changed files: the owner, pet, and visit controllers; transactional clinic-record service and typed duplicate exception; exact Thymeleaf actions in the owner, pet, and visit forms; controller, concurrency, and real-server tests; status and this checkpoint.
- Approved UCs regression-tested: UC-1 through UC-7 in the 373-test suite.

## Notes

The first focused attempts were environment-only failures before UC behavior ran: Java 21 required the established Byte Buddy agent, and the restricted sandbox refused localhost server binding. The successful focused command used the agent and permitted local random ports.

Convergence finding C-1 showed that the original real-server client borrowed the navbar logout token, masking missing CSRF fields in the owner, pet, and visit forms. The revision gives each form its exact Thymeleaf POST action and requires the real-server test to find the token inside the submitted form. The revision focused suite passed 38 tests and the full suite passed 373 tests, both with zero failures, errors, or skips.

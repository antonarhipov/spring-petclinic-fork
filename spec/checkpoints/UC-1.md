...
- Prior convergence: `04e5d3b` rejected C-1 and noted K-1; both are resolved in this submission
...
| UC-1 G7 | Staff pet/visit form labels and actions resolve through message keys at `createOrUpdatePetForm.html:20` and `createOrUpdateVisitForm.html:32`; `I18nPropertiesSyncTest.java:85` scans template and Java visible text and verifies exact key parity across all eleven locale bundles. | PASS |
...
| UC-1 G10 | `I18nPropertiesSyncTest.java:85`, `I18nPropertiesSyncTest.java:116`, and `I18nPropertiesSyncTest.java:174` enforce message-key use, all-bundle parity, and explicit failing fixtures for direct, text-assignment, and fragment-label Thymeleaf literals. | PASS |
...
| RULE-17 | `I18nPropertiesSyncTest.java:85` enforces exact bundle parity and rejects hard-coded visible template, Thymeleaf-expression, or Java messages; all eleven bundles contain the added action keys. | PASS |
...
- Full relevant suite: `JAVA_HOME=/Users/anton/Library/Java/JavaVirtualMachines/jbr-21.0.8/Contents/Home ./mvnw -q -Dspring-javaformat.skip=true -DargLine=-javaagent:/Users/anton/.m2/repository/net/bytebuddy/byte-buddy-agent/1.18.10/byte-buddy-agent-1.18.10.jar test` - PASS, 102 tests, 0 failures, 0 errors, 0 skipped.
...
- Changed files: the complete implementation file set is recorded under `UC-1 Evidence` in `spec/status.md`; it consists of the Maven/H2 support boundary, Flyway migrations and seeds, authentication and owner-scoping code, shared templates/localization, removal of unsupported build/database/deployment paths, and UC-1 verification tests. The revision additionally changes `README.md`, the pet and visit forms, all eleven bundles, and `I18nPropertiesSyncTest.java`.
- Approved UCs regression-tested: none; UC-1 is the first use case.

## Notes

The ambient-JDK diagnostic run failed because JDK 25 could not self-attach Mockito. All reported verification used the project's required JDK 21 and an explicit Byte Buddy agent. Convergence C-1 was resolved by removing visible Thymeleaf expression literals and adding a scanner regression fixture; K-1 was resolved by documenting `./mvnw test`. Human verification remains limited to the UC-1 G9 owner/staff walkthrough required by convergence.

...
- Submission: `spec/checkpoints/UC-1.md` at `e0f330e01b596044a3cd9c2031a0bdef4d86da61`
- Verdict: REJECT
- Findings: 1 critical, 0 gaps, 0 protocol, 1 cosmetic
- Suite: 101 run, 0 failed, 0 errors, 0 skipped
...
2. PASS - `spec/checkpoints/UC-1.md` and the submitted implementation are committed together at `e0f330e01b596044a3cd9c2031a0bdef4d86da61`; the base is `2b8fe6fc52c133b9bdbc3a06026f2903ff00fc43`.
...
5. PASS - The checkpoint maps the main scenario, all ten extensions, all ten guarantees, both postconditions, and every applicable rule to evidence and records commands, the implementation file set, and the absence of approved-UC regressions.
6. PASS - Inspection of all 96 changed paths found UC-1 authentication, authorization, localization, presentation, Flyway/normative seed, H2/Maven-boundary work, supporting tests, and required removals. No later use-case action is exposed beyond the owner-scoped state-only status resource required by UC-1/RULE-14.
...
| UC-1 G7 | All visible text resolves through keys in eleven bundles. | Several staff form labels and actions bypass message keys at `createOrUpdatePetForm.html:20-27` and `createOrUpdateVisitForm.html:32-33`. | STRONG | no |
...
| UC-1 G9 | Owner/staff rendered walkthrough is required before acceptance. | Automated DOM evidence passed; human walkthrough remains intentionally pending until automated findings are resolved. | STRONG | pending walkthrough |
| UC-1 G10 | Localization scanning fails on bypassed messages or incomplete bundles. | Bundle equality works, but `I18nPropertiesSyncTest.inspectHtml` ignores user-visible string literals inside Thymeleaf expressions, allowing the G7 violations above to pass. | STRONG | no |
...
| RULE-17 | All visible template and server text must use keys present in all eleven bundles. | Hard-coded staff-form labels and action text remain, and the scanner does not detect Thymeleaf expression literals. | FAIL - C-1 |
| RULE-18 | Real-server journey and direct evidence for every contract element are required without test data impact. | The real actor journey and negative evidence are strong, and verification left no runtime/tracked data; G7/G10 remain incorrect as described in C-1. | FAIL - C-1 |
| RULE-24 | Only Maven/H2 support may remain and README must document the boundary. | Repository/dependency inventory, runtime/test properties, and README assertions pass; unsupported database/build/deployment paths are absent. | PASS WITH K-1 |
...
### C-1 CRITICAL - Staff forms bypass complete localization and the scanner misses them

- References: UC-1 G7 requires, “All user-visible text ... resolve through message keys present in all eleven shipped locale bundles”; G10 requires localization evidence that “fails when ... an application message bypasses a key”; RULE-17 says all visible template text “MUST” resolve from keys and names source/template scans as verification.
- Evidence: `src/main/resources/templates/pets/createOrUpdatePetForm.html:20-27` passes `Name`, `Birth Date`, and `Type` as literal labels and selects literal `Add Pet`/`Update Pet` button text. `src/main/resources/templates/pets/createOrUpdateVisitForm.html:32-33` passes literal `Date` and `Description` labels. `src/main/resources/templates/fragments/inputField.html:10` and `selectField.html:10` render those arguments directly. `I18nPropertiesSyncTest.java:161-189` scans plain HTML text, a limited attribute set, and message references but never rejects visible literals inside `th:replace`, `th:with`, or `th:text` expressions.
- Why it fails: these authenticated staff pages render fixed English even under another locale, and the required automated safeguard passes for an implementation it must reject.
- Revision outcome: replace every user-visible expression literal with existing or new message keys in all eleven bundles, and extend automated localization evidence so a visible literal inside a Thymeleaf expression fails the test without confusing internal field names, route fragments, CSS tokens, or date formats for UI copy.

### K-1 COSMETIC - README names a missing test wrapper

- Evidence: `README.md:37` instructs `scripts/test-with-clean-tree-check.sh clean test`, but that path does not exist in the submission.
- Why it matters: this does not change UC-1 runtime behavior or required RULE-24 documentation, but the documented test command cannot be run.
- Revision outcome: document the real Maven test command or add the described wrapper when an in-scope implementation owns it.

## Walkthrough

Deferred. UC-1 G9 must be presented only after the automated localization finding is resolved and convergence passes.

## Status Update

`READY_FOR_CONVERGENCE` -> `NEEDS_REVISION`; no later use case is eligible.

## Response to execute

REVISE UC-1: remove hard-coded staff-form text, make localization scanning detect visible Thymeleaf expression literals, and rerun all UC-1 evidence before walkthrough.

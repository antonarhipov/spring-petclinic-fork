...
class OwnerRecoveryJourneyTests {
	...
	void rejectThenReviseThenWithdraw() throws Exception {
		...
		Offer first = this.offers
			.findFirstByRequestRevisionIdAndStatusOrderByCreatedAtDesc(request.getActiveRequestRevisionId(),
					OfferStatus.HELD)
			.orElseThrow();
		request = this.requests.findById(request.getId()).orElseThrow();
		this.mockMvc
			.perform(post("/owner/scheduling-requests/{id}/offers/{offerId}/reject", request.getId(), first.getId())
				.with(csrf())
				.param("expectedVersion", request.getVersion().toString()))
			.andExpect(status().is3xxRedirection());
		request = this.requests.findById(request.getId()).orElseThrow();
		...
		this.mockMvc
			.perform(post("/owner/scheduling-requests/{id}/revise-request", request.getId()).with(csrf())
				.param("expectedVersion", request.getVersion().toString())
				.param("sourceText", "Leo still needs a wellness exam but later in the week"))
			.andExpect(status().is3xxRedirection());
		request = this.requests.findById(request.getId()).orElseThrow();
		assertThat(request.getState()).isEqualTo(RequestState.AWAITING_CONSENT);
		this.mockMvc
			.perform(post("/owner/scheduling-requests/{id}/withdraw", request.getId()).with(csrf())
				.param("expectedVersion", request.getVersion().toString()))
			.andExpect(status().is3xxRedirection());

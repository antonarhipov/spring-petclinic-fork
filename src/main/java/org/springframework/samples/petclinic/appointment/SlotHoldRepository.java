...
/**
 * Repository for {@link SlotHold} domain objects.
 */
public interface SlotHoldRepository extends JpaRepository<SlotHold, Integer> {

	@Query("SELECT h FROM SlotHold h WHERE h.vet.id = :vetId AND h.expiresAt > :now")
	List<SlotHold> findActiveByVetId(@Param("vetId") Integer vetId, @Param("now") Instant now);
